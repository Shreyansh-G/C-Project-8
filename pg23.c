...
Author: Shreyansh_G
Date:28/12/2023
Project:Modify the singly linked list to be a doubly linked list. 
...
// defines the double linked list
typedef struct list{ int data; struct list *next; struct list *prev;} list;

// This method returns 1 or 0 based on whether or not argument
...
// This method takes an integer and a list,
// creates a new head with argument data and makes it
...
// This method takes an array, initializes a new list with arrays
//elements as elements and
// returns list head
...
// This method prints the title of parameter list, followed by as
//line of this list's elements
...
// This method swaps the data of two given pointers to different
...
i will represent the number of times we will move the
pointers one step ahead
decreasing as we bubble more and more element to the end
of the list
*/
     for(i = SIZE-2; i >= 0; i--) {
            p1 = h;
...
if(node != NULL){
// If the element we want to remove is the only element in
//the list
...
void remove_dups(list *h){
   ...
   // The removal will "push" the list back
    ...
    // If the elements are different, push the pointer one step
     h = h->next;
     }
   }
}    
